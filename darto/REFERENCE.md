...
await app.listenSecure(443, ctx); // HTTPS convenience

await app.stop();                                  // graceful drain
...
By default `serve` / `listen` trap `SIGINT`/`SIGTERM` and shut down gracefully
(stop accepting, drain in-flight requests, then exit) — pass
`shutdownSignals: false` to opt out.
...
> Literal routes (no params or wildcards) are matched by a direct string
> compare on the dispatch hot path — only dynamic routes fall back to the
> compiled matcher — and dispatch short-circuits on the HTTP method first.
...
app.use(requestId(headerName: 'X-Trace-Id'));  // custom header

app.get('/', [], (c) => c.ok({'id': requestIdOf(c)}));
```

Honors an incoming header when present; otherwise generates a UUID v4. The id is
stored in context (`requestIdOf(c)`) and echoed in the response header.
...
app.use(etag());            // strong validator
app.use(etag(weak: true));  // weak validator: W/"…"
```

Hashes dynamic responses, sets the `ETag` header, and returns `304 Not Modified`
when the client's `If-None-Match` matches. (`darto_static` already handles ETag
for static files.)
...
// Per-user limit, skip health checks, custom rejection
...
Emits the IETF draft `RateLimit-Limit` / `RateLimit-Remaining` / `RateLimit-Reset`
headers and `Retry-After` on rejection. The default `MemoryRateLimitStore` is
process-local; implement `RateLimitStore` for a shared/distributed backend.
...
Returns `200 {"status":"ok"}` when every check passes, or `503` with the failing
checks (a throwing check counts as down).
...
Pair it with [zard](https://pub.dev/packages/zard) for schema-driven validation with full control over the error response. You only need the `zard` package — **not** `darto_validator` (that package is only for the higher-level `zValidator`, see [Validation](#validation)):

```dart
import 'package:darto/darto.dart';
import 'package:darto/validator.dart';
import 'package:zard/zard.dart'; // add `zard` to your pubspec
