...
final getUserParams = z.map({
  'id':
      z.coerce.int().min(1).openapi(description: 'ID do usuário a ser buscado'),
}).openapiSchema();

void main() async {
  // 1. Your own Darto app + the OpenAPI plugin on top (composition).
  final app = Darto();
  ...
  api.openapi(getUserRoute, [], (c) {
    final id = c.req
        .valid<Map<String, dynamic>>('param')['id']; // int, coercido+validado
  ...
  api.openapi(createUserRoute, [], (c) {
    final body =
        c.req.valid<Map<String, dynamic>>('json'); // validado pelo zard
